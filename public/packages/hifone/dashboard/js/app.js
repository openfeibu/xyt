	...
	admin.prototype = {
		...
		viewModel: {
			...
			clickItem: function(id)
			{
				...
				{
					if(page_type == 'config'){
						window.location.href= route + this.modelName() + '/' + id ;
					}
					else{			
			...
			clickCategoryItem: function(id,parentid)
			{
				if (!this.loadingItem() && this.activeItem() !== id && this.actionPermissions.view)
				{
					
					History.pushState({modelName: this.modelName(), id: id}, document.title, route + this.modelName() + '/' + id);
				//	$("#edit_field_"+is_category).focus().val(pid);
					console.log(adminData.edit_fields.pid.value);
					this.viewModel.originalEditFields.pid.value = parentid;
				}
			},
			/**
			...
			setData: function(data)
			{
				...
				if(data.data_model && data.data_model.length != 0){
					adminData.data_model = data.data_model;
					self.model = adminData.data_model;
					ko.mapping.updateData(self, self.model, self.model);
				}
