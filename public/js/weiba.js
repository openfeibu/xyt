...
M.addEventFns({
	...
	do_weiba_reply:{	//回复帖子操作
		click:function(){
			...
			attrs.post_user_id = $(this).attr('post_user_id');
            ...
            if (typeof attach_id != 'undefined') {
     		...
     		$.post(ADD_REPLY_URL,{post_id:attrs.post_id,post_user_id:attrs.post_user_id,to_reply_id:attrs.to_reply_id,to_user_id:attrs.to_user_id,content:content,  list_count:attrs.list_count,post_from:attrs.post_from,space_id:attrs.space_id,addtoend:attrs.addtoend},function(msg){
...
//						$(commentListObj).find('.comment_lists').eq(0).html(msg.data);
	...
	reply_del:{
		click:function(){
			var attrs = M.getEventArgs(this);
			$.post(DELETE_REPLY_URL,{widget_appname:'weiba',reply_id:attrs.reply_id},function(msg){
				$('#reply_count').html(parseInt($('#reply_count').html()) - 1);
