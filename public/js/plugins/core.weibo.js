...
//			$('#feed-lists').append("<div class='loading' id='loadMore'>" + L('PUBLIC_LOADING') + "<img src='" + THEME_URL + "/images/load.gif' class='load'></div>");
		...
		$(window).bind('scroll resize', function() {
			...
			if(bodyTop + $(window).height() >= bodyHeight - 250) {
				...
				if($('#feed-lists').length > 0 && _this.loadId != null){
					$('#feed-lists').append("<div class='loading' id='loadMore'>" + L('PUBLIC_LOADING') + "<img src='" + THEME_URL + "/images/ico-load.png' class='load'></div>");
					core.weibo.loadMoreFeed();
				}
			}
		});
...
//					_this.startNewLoop();
	...
	loadMoreByPage: function(href) {
		...
		$('#feed-lists').html("<div class='loading' id='loadMore'>" + L('PUBLIC_LOADING') + "<img src='" + THEME_URL + "/images/ico-load.png' class='load'></div>");
	...
	insertToList: function(html, feedId) {
		...
		if($('#feed-lists').length > 0) {
			...
			$dl.addClass('space_content  feed_list');
			$dl.html(html);
			if(before.length > 0) {
		...
		if($('#feed-lists-d').length > 0) {
			...
			_dl.setAttribute('class', 'feed_list space_content');
	...
	checkNums: function(obj, post) {
		...
		} else if (typeof obj.parentModel.parentModel.childModels['numsLeft'] != 'undefined') {
			$numsLeft = obj.parentModel.parentModel.childModels['numsLeft'];
