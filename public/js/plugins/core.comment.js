...
core.comment = {
	...
	display: function(callback) {	
		...
		if(commentListObj.style.display == 'none') {
			...
			if(commentListObj.innerHTML !=''){
				...
				$.post(COMMENT_RENDER_URL,{app_uid:this.app_uid,row_id:this.row_id,app_row_id:this.app_row_id,app_row_table:this.app_row_table,isAjax:1,showlist:0,
						...
						canrepost:this.canrepost },function(html){
							if(html.status =='0'){
								...
								$.post(COMMENT_LIST_URL,{app_name:appname,table:table,row_id:rowid,cancomment:cancomment},function (res){
	...
	delComment:function(comment_id){
		$.post(DELETE_COMMENT_URL,{comment_id:comment_id},function(msg){
			// 动态添加字数
			var commentDom = $('#feed'+core.comment.row_id).find('a[event-node="comment"]');
			var oldHtml = commentDom.html();
			if (oldHtml != null) {
