...
core.share = {
	...
	post_share: function(_this, mini_editor, textarea) {
		...
		$.post(ADD_SHARE_SPACE_URL,{body:data,type:attrs.type,app_name:attrs.app_name,sid:attrs.sid,content:'',comment:ifcomment,curid:attrs.curid,curtable:attrs.curtable},
			function(msg){
			if(msg.status == 1){
				updateUserData('weibo_count',1);
				if(MID == UID){
					// console.log(msg);
					core.weibo.insertToList(msg.data, msg.space_id);
				}
