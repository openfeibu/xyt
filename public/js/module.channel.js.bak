...
M.addEventFns({
	...
	delFeed:{
		click:function(){
			...
			var _delFeed =  function(){
				$.post(U('public/Feed/removeFeed'),{space_id:attrs.space_id},function(msg){
					if(msg.status == 1){
						if($('#feed_'+attrs.space_id).length > 0){
							$('#feed_'+attrs.space_id).fadeOut();
						}else{
						...
						if (typeof attrs.callback === 'string') {
							eval(attrs.callback + '(' + attrs.space_id + ')');
						}
	...
	loadPost:{
		click:function(){
			...
			if($(_this).parent().parent().find('.feed_img_lists').css('display') == 'none'){
				$(_this).parent().parent().find('.feed_img_lists').before('<dl id="loading" class="comment"><div class="loading" style="z-index:99;">加载中<img src="'+THEME_URL+'/images/load.gif" class="load"></div></dl>');
			};
			$.post(U('widget/FeedList/getPostDetail'),{post_id:attrs.post_id},function(res){
				...
				if(res == 0){
					...
					html += '<div><a href="javascript:void(0);" onclick="$(\'#post_'+attrs.space_id+'_'+attrs.post_id+'\').slideToggle()" class="mr10"><i class="ico-pack-up mr10"></i>收起</a><a target="_blank" href="'+res.post_url+'"><i class="ico-show-big"></i>查看原文</a></div>';
					...
					html += '<div><a href="javascript:void(0);" onclick="$(\'#post_'+attrs.space_id+'_'+attrs.post_id+'\').slideToggle();window.location.hash=\'#feed'+attrs.space_id+'\';window.location=window.location;" class="mr10"><i class="ico-pack-up"></i>收起</a><a target="_blank" href="'+res.post_url+'"><i class="ico-show-big"></i>查看原文</a></div>';
				}
				html += '</div>';
				$('#loading').remove();
	...
	show_admin: {
		...
		click: function() {
			...
			var html = $('#list_html_' + args.space_id).html();
...
{
	ui.box.load(U('channel/Manage/getAdminBox')+'&space_id='+feedId+'&channel_id='+channelId+'&clear='+clear, '推荐到频道');
};
/**
 * 添加微事务窗口
...
var addToVtask = function(feedId) {
	ui.box.load(U('vtask/Index/addToVtask') + '&space_id=' + feedId, '添加到微事务');
};
/**
 * 推荐动态
 * @param integer feedId 分享ID
...
var feed_recommend = function(feedId, val) {
	$.post(U('public/Feed/feed_recommend'),{space_id:feedId,val:val},function(){
		ui.success("推荐成功");
		//window.location.href = window.location.href;
	});
};
...
var addFeedHomeTop = function(uid, feedId, isrefresh) {
	...
	$.post(U('public/Widget/addonsRequest'), {'space_id':feedId, 'uid':uid, 'addon':'FeedTopHome', 'hook':'add_feed_top_home'}, function(res) {
		if (res.status) {
			ui.success(res.info);
			if (isrefresh == 1) {
				setTimeout(function() {
...
var delFeedHomeTop = function(uid, feedId, isrefresh) {
	...
	$.post(U('public/Widget/addonsRequest'), {'space_id':feedId, 'uid':uid, 'addon':'FeedTopHome', 'hook':'del_feed_top_home'}, function(res) {
		if (res.status) {
			ui.success(res.info);
			if (isrefresh == 1) {
				setTimeout(function() {
...
var addFeedTop = function(uid, feedId, isrefresh) {
	...
	$.post(U('public/Widget/addonsRequest'), {'space_id':feedId, 'uid':uid, 'addon':'FeedTop', 'hook':'add_feed_top_home'}, function(res) {
		if (res.status) {
			ui.success(res.info);
			if (isrefresh == 1) {
				setTimeout(function() {
...
var delFeedTop = function(uid, feedId, isrefresh) {
	...
	$.post(U('public/Widget/addonsRequest'), {'space_id':feedId, 'uid':uid, 'addon':'FeedTop', 'hook':'del_feed_top_home'}, function(res) {
		if (res.status) {
			ui.success(res.info);
			if (isrefresh == 1) {
				setTimeout(function() {
