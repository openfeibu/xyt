...
<div class="b_j">
	<div class="b_k b_k_pass">
        ...
        <div class="common_main font12">
	        @if(isset($mobile))
	        <form method="post" action="{{route('user.forget_password_mobile_next')}}" id="test" name="">
			<table  border="0"  class="table">
              ...
              <tr>
                ...
                <td><input name="mobile" type="text"	id="mobile" class="pinput" value="{{$mobile}}"/><input  type="button" id="sendforgetcode" class="commonbtn" value="获取验证码" style="margin-left:10px"/><span id="sendmsg"></span></td>
              </tr>
              <tr>
	        ...
	        @else
	        <form method="post" action="{{url('/user/forgetPassword')}}" id="test">	
...
$("#sendforgetcode").on('click',function(){
	...
	if(reg.test(mobile)){
		...
		$.get("{{route('user.forget_password_mobile')}}"+'?mobile='+mobile,function(data){
			if(data.code==200){
