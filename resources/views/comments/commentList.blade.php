...
<dl class="b_nc" class="comment_list"  model-node="comment_list">
	...
	<dd class="b_ne" style="margin-top:0px">
		<a href="javascript:void(0)" event-args='row_id={{$vo['row_id']}}&app_uid={{$vo['app_user_id']}}&to_comment_id={{$vo['id']}}&to_uid={{$vo['user_id']}}&to_comment_uname={{$vo['user_info']['username']}}&app_name={{$app_name}}&table={{$table}}' event-node="reply_comment">{{trans('public.PUBLIC_STREAM_REPLY')}}</a>&nbsp;
		@if(Auth::id() == $vo['user_info']['id'] || Auth::user()->can("manage_comment"))
		<a href="javascript:void(0);" event-node="comment_del" event-args="comment_id={{ $vo['id'] }}">{{ L('PUBLIC_STREAM_DELETE') }}</a>&nbsp;
		@endif
	</dd>
</dl>
@endforeach 
