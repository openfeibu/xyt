    ...
    <div class="activity">
        ...
        <div class="activity_nav">
            ...
            <ul class="activity_nav_ul2">
                <a href="{{route('activity.u')}}"><li style="text-align: right;">全部&nbsp;|&nbsp;</li></a>
                <a href="{{route('activity.u',['type' => 'join'])}}"><li>参加的活动&nbsp;&nbsp;|&nbsp;</li></a>
                <a href="{{route('activity.u',['type' => 'follow'])}}"><li>关注的活动&nbsp;&nbsp;|&nbsp;</li></a>
                <a href="{{route('activity.u',['type' => 'create'])}}"><li>组织的活动</li></a>
            </ul>
        </div>
        ...
        <div class="activity_place">
            ...
            <div class="activity_place_right fleft">
                <ul>
                    <a href="{{route('activity.u',['type' => $type,'cat_id' => $cat_id])}}"><li @if(!$province_id)class="selected"@endif>全部</li></a>
                    @foreach($provinces as $key => $province)
                    <a href="{{route('activity.u',['type' => $type,'province_id' => $province->id,'cat_id' => $cat_id])}}" ><li @if($province->id == $province_id)class="selected"@endif>{{$province->title}}</li></a>
                    @endforeach
                </ul>
            </div>
        </div>
        ...
        <div class="activity_type">
            ...
            <div class="activity_type_right fleft">
                <ul>
                    <a href="{{route('activity.u',['type' => $type,'province_id' => $province_id])}}"><li @if(!$cat_id)class="selected"@endif>全部</li></a>
                    @foreach($cats as $key => $cat)
                    <a href="{{route('activity.u',['type' => $type,'province_id' => $province_id,'cat_id' => $cat->id])}}"><li @if($cat->id == $cat_id)class="selected"@endif>{{$cat->title}}</li></a>
                    @endforeach
                </ul>
            </div>
        </div>
