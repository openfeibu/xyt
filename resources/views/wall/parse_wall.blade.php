<div class="list" id="item_{{$reply_id}}">
	<dl class="b_nc" model-node="comment_list">
		...
		<dd class="b_ne" style="margin-top:0px">
			<a href="javascript:void(0)" event-args="post_id={{$post_id}}&to_reply_id={{$reply_id}}&to_user_id={{$user_id}}&to_comment_uname={{$user_info['username']}}&id={{$reply_id}}&addtoend={{$addtoend}}" event-node="reply_reply">{{trans('public.PUBLIC_STREAM_REPLY')}}</a>&nbsp;
