...
<tr><td rowspan="4" width="120px"><a href="{{route('user.home',$user->id)}}"><img src="{{$user->avatar}}?v={!!time()!!}" width="95" height="118" alt="" title="" /></a></td><td><span class="green">姓名:</span>{{$user->username}}</td><td><span class="green">居住地:</span> {{$user->location}}</td><td><span class="green">职业:</span> {{$user->work}}</td></tr>
...
<tr><td colspan="3"><a href="{{route('user.home',['id' => $user->id])}}" class="b_kb">会员的详细资料及择偶要求</a>（会员号：{{$user->id}}）</td></tr>
