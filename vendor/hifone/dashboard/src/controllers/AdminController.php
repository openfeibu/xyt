...
class AdminController extends Controller
{
    ...
    */
    ...
    {
	    //Frozennode\Administrator\Config\Model
        ...
        if ($this->request->ajax()) {
            ...
            if ($model->exists) {
                $model = $page_type == 'config' ? $config->updateConfigModel($model, $fieldFactory, $actionFactory, $itemId) : $config->updateModel($model, $fieldFactory, $actionFactory);
            }
	        ...
	        if($page_type == 'category'){
	           	$view = view('administrator::category', array(
	                'itemId' => $itemId,
	            ));
            }
			else{
    ...
    public function save($modelName, $id = false)
    {
        ...
        $save = $page_type == "config" ? $config->saveConfig($this->request, $fieldFactory->getEditFields(), $actionFactory->getActionPermissions(), $id) : $config->save($this->request, $fieldFactory->getEditFields(), $actionFactory->getActionPermissions(), $id);
        ...
        if (is_string($save)) {
            ...
            if ($model->exists) {
                $model = $page_type == "config" ? $config->updateConfigModel($model, $fieldFactory, $actionFactory, $id) : $config->updateModel($model, $fieldFactory, $actionFactory);
            }
    ...
    public function results($modelName)
    {
        ...
        //return the rows
        if($page_type == 'category'){
        	return response()->json($dataTable->getCategorysRows(app('db'), $filters, $page, $sortOptions));
    	}else{
