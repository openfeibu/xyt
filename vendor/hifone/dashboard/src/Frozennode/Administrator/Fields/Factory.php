...
class Factory
{
    ...
    /*������Ҫ�Ż�  -- ��β�ͱ���*/
	public function getConfigEditFieldsArrays ($id)
	{
		$system_datas = $this->getSystemDatas($id);
		$configData = $this->config_get($system_datas->key);
		...
		foreach( $configData['key'] as $key => $value )
		{
			$arrayFields[$key] = array(
		        ...
		        "visible" => true,
		        "setter" => false,
		        "description" => $configData['key_description'][$value] ,
		        //"value" =>  $datas[$key] ? $datas[$key] : $configData['key_default'][$value],
		     	"value" => "",
		        "min_value" => "",
		        "max_value" => "",
		        "min_max" => false,
		        "limit" => 0,
        		"height" => 100,
		        "title" =>   $configData['key_name'][$value],
		        "field_name" => $key,
		        "hint" => "",
		        "type" => 'text',
			) ;
	...
	public function getSystemDatas ($id)
	{
		$system_datas = $this->db->table('system_datas')->select('key','value')->where('id',$id)->first();
		return $system_datas ;
	}
    /**
     ...
     * @return array
     */
    public function getConfigDataModel($id)
    {
        ...
        foreach( $configData['key'] as $key => $value )
        {
        	//$configModel[$key] = $datas[$key] ? $datas[$key] : $configData['key_default'][$value];
        	$configModel[$key] = '' ;
        }

        return array_merge($dataModel,$configModel);
    }
    /**
    ...
    public function config_get ($key)
	{
        ...
        $list = $this->config_lget();

        return $list ? $list[$key] : '';
	}

	public function config_lget($list_name = 'default', $nostatic = false)
    {
        $list_name = $this->_strip_key($list_name);

        $cache_id = 'system_config_lget_'.$list_name;

		if (Cache::has($cache_id)) {
		   $data = Cache::get($cache_id);
		}
		else{
		    $result = $this->db->table('system_config')->where('list',$list_name)
	        			->orderBy('id','desc')
	        			->get();
			foreach ($result as $v) {
	            $data[$v->key] = unserialize($v->value);
	        }
			Cache::put($cache_id, $data, "60");
       	}
        $_res[$list_name] = $data;
