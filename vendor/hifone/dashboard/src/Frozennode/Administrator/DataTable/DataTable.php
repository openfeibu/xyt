...
class DataTable
{
	...
	public function getCategorysRows(DB $db, $filters = null, $page = 1, $sort = null)
	{	
        ...
        $output = $this->performCountQuery($countQuery, $querySql, $queryBindings, $page,$where);
        ...
        $output['results'] = $this->parseResults($query->where('pid','=',0)->get());
     ...
     * @return array
     */
    public function performCountQuery(QueryBuilder $countQuery, $querySql, $queryBindings, $page,$where = null)
    {
        //grab the model instance
        $model = $this->config->getDataModel();

        //then wrap the inner table and perform the count
        $sql = "SELECT COUNT({$model->getKeyName()}) AS aggregate FROM ({$querySql}) AS agg $where";
