...
class SystemDateRepository{
    ...
    public function lput($listName = '', $listData = array())
    {
        ...
        if (is_array($listData)) {
            $insert_sql    .=    'REPLACE INTO system_datas (`list`,`key`,`value`,`mtime`) VALUES ';
            foreach ($listData as $key => $data) {
                $insert_sql    .= " ('$listName','$key','".addslashes(serialize($data))."','".date('Y-m-d H:i:s')."') ,";
            }
            $insert_sql = rtrim($insert_sql, ',');
            // 插入数据列表
            $result = DB::statement($insert_sql);
        }
    ...
    public function lget($list_name = '', $nostatic = false)
    {
        ...
        if (($data = S($cache_id)) === false || $data == '') {
            ...
            $result = SystemOrder::where('list',$list_name)->orderBy('id', 'ASC')->get();
    ...
    public function put($key, $value = '', $replace = true)
    {
        ...
        if ($replace) {
            $insert_sql = 'REPLACE INTO system_datas ';
        } else {
            $insert_sql = 'INSERT INTO system_datas ';
        }
